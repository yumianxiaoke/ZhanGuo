...
public class LuaScriptMgr
{
    ...
    public void ReloadAll()
    {
        ...
        Debugging.Log("Reload lua files over");
    }
    ...
    void PrintLua(params string[] param)
    {
        if (param.Length != 2)
        {
            Debugging.Log("PrintLua [ModuleName]");
            return;
        }
    ...
    {        
        ...
        Debugging.Log("Lua module destroy");        
    }
    ...
    public LuaFunction GetLuaFunction(string name)
    {
        ...
        if (!dict.TryGetValue(name, out func))
        {
            ...
            {
                Debugging.LogWarning("Lua function {0} not exists", name);
            }
    ...
    public byte[] Loader(string name)
    {
        ...
        else
        {
            ...
            string path = "Lua/Out/" + fileName;
            TextAsset ta = ResourcesManager.Instance.Load<TextAsset>(path);
            str = ta.bytes;
        }
#else
    ...
    public static void DumpStack(IntPtr L)
    {
        ...
        for (int i = 1; i < top; i++)
        {
            ...
            {
                case LuaTypes.LUA_TSTRING:
                    Debugging.Log(LuaDLL.lua_tostring(L, i));
                    break;
                case LuaTypes.LUA_TBOOLEAN:
                    Debugging.Log(LuaDLL.lua_toboolean(L, i).ToString());
                    break;
                case LuaTypes.LUA_TNUMBER:
                    Debugging.Log(LuaDLL.lua_tonumber(L, i).ToString());
                    break;                
                default:
                    Debugging.Log(LuaDLL.lua_typename(L, t));
                    break;
            }
        }
    }
